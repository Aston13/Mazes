...
/**
 * Handles all in-game rendering: background, maze tiles, player sprite,
 * HUD, collision detection, and animation frames.
 */
public class Renderer {
    ...
    public Renderer(int screenHeight, int screenWidth, int rowColAmount,
                    int tileWH, AssetManager assetManager, MazeGame game) {
        ...
        gameTimer = new Timer(TIMER_TICK_MS, (ActionEvent e) -> {
    ...
    /**
     * Renders all visible maze tiles, including grass backgrounds,
     * key items, wall/exit sprites, and exit accessibility checks.
     *
    ...
    public void renderMaze(Graphics g, int tileWH) {
        for (int row = 0; row < rowColAmount; row++) {
            for (int col = 0; col < rowColAmount; col++) {
                ...
                if (tile.getMinX() <= -tileWH || tile.getMaxX() >= screenWidth + tileWH
                        || tile.getMinY() <= -tileWH || tile.getMaxY() >= screenHeight + tileWH) {
                    continue;
                }

                // Grass background
                String grassVariant = (col % 2 == 0)
                        ? "GrassPassage_" + tile.getPassageImageId()
                        : "GrassPassage_0";
                g.drawImage(getImage(grassVariant),
                        tile.getMinX(), tile.getMinY(), tile.getSize(), tile.getSize(), null);

                // Key item animation
                if ("Key".equals(tile.getImageString())) {
                    BufferedImage keyFrame = (keysOnMap.peek() == tile)
                            ? assetManager.getBlinkingKeyFrame()
                            : assetManager.getKeyFrame();
                    if (keyFrame != null) {
                        g.drawImage(keyFrame,
                                tile.getMinX(), tile.getMinY(), tile.getSize(), tile.getSize(), null);
                    }
                }
    ...
    /**
     * Scrolls the maze horizontally and queues the appropriate
    ...
    /**
     * Scrolls the maze vertically and queues the appropriate
     * player walking animation.
     *
     * @param g            the graphics context
     * @param numOfRowCol  grid size
    ...
    /**
     * Checks collision at the given tile position and handles game events
     * (key pickup, exit check, wall blocking).
     *
    ...
    /**
     * Renders the player sprite at the centre of the screen,
     * plus any active player message.
     *
     * @param g          the graphics context
    ...
    public void renderPlayer(Graphics g, Player player, int spriteSize) {
        ...
        if (isMessageVisible()) {
            ...
            g.drawString(playerMessage,
                    (screenWidthHalf - halfTextWidth) + tileWidth / 2, screenHeightHalf);
        }
    }
